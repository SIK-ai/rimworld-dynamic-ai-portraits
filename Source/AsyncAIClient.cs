...
namespace AIPortraits
{
    ...
    public static class AsyncAIClient
    {
                                                 ...
                                                 AIPortraitsSettings settings, PawnState state, byte[] portraitBytes, PortraitCallback callback)
        {
            ...
            {
                if (Prefs.DevMode) Log.Message("[Dynamic AI Portraits] No API key for " + bt + " — using free Pollinations.");
                                                            ...
                                                            string continuityToken, byte[] portraitBytes, PortraitCallback callback)
        {
            ...
            using (UnityWebRequest request = new UnityWebRequest(llmUrl, "POST"))
            {
                ...
                if (Prefs.DevMode) Log.Message("[Dynamic AI Portraits] Calling " + modelLogName + " for " + (state.name ?? "pawn") + "...");
                yield return request.SendWebRequest();
        ...
        private static string ExtractGeminiText(string json)
        {
            ...
            catch (Exception ex) { if (Prefs.DevMode) Log.Warning("[Dynamic AI Portraits] Ignored exception: " + ex); return null; }
        }
            ...
            if (state != null)
            {
                ...
                else if (state.framing == "special")
                {
                    ...
                    if (ImageConversion.LoadImage(temp, imgBytes))
                    {
                        ...
                        {
                            shouldRemoveBg = true;
                            if (Prefs.DevMode) Log.Message("[Dynamic AI Portraits] Detected mono-colored background in special photoshoot. Enabling background removal.");
                        }
                    }
