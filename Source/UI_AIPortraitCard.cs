...
namespace AIPortraits
{
    ...
    public static class AIPortraitsManager
    {
        ...
        // Optimization: Using PawnFramingKey struct to eliminate per-frame string allocation GC pressure
        ...
        public static Texture2D GetPortraitTexture(Pawn pawn, out GenerationStatus status, out string error)
        {
            ...
            {
                PawnFramingKey lockedCacheKey = new PawnFramingKey(pawn.thingIDNumber, GetActiveFraming(pawn) + LockedSuffix);
            ...
            foreach (string f in allFramings)
            {
                ...
                if (loadedTextures.ContainsKey(fPawnKey) || loadedTextures.ContainsKey(new PawnFramingKey(pawn.thingIDNumber, f + LockedSuffix))) { hasAnyPortrait = true; break; }
        ...
        public static void TriggerNewPortraitWithContinuity(Pawn pawn)
        {
            ...
            PawnFramingKey lockedCacheKey = new PawnFramingKey(pawnKey.pawnId, pawnKey.framing + LockedSuffix);
            Texture2D oldTex;
        ...
        public static void ClearPawnActiveTextureCache(Pawn pawn)
        {
            if (pawn == null) return;
            string framing = GetActiveFraming(pawn);
            PawnFramingKey cacheKey = new PawnFramingKey(pawn.thingIDNumber, framing + LockedSuffix);
            Texture2D tex;
